...
for(i in label){
  ...
  for(y in years){
    if(y!=2014){
      #On somme les augmentations et les diminutions de la durée des itinéraires supérieures/inférieures à x% par distance au réseau et par année puis on divise par le nombre de traces (5000) et on transforme en pourcentage
      d1Dist[nbLigne,nbCol]<-(sum(data$city==i & data$year==y & data$pctDiffDis>=1)+sum(data$city==i & data$year==y & data$pctDiffDis<=-1))/10
      ...
      d20Dist[nbLigne,nbCol]<-(sum(data$city==i & data$year==y & data$pctDiffDis>=20)+sum(data$city==i & data$year==y & data$pctDiffDis<=-20))/10
...
#Orléans  
assign(paste("boxplotDist","Orleans",sep=""),(ggplot(o, aes(x=year, y=pctDiffDis)) + geom_boxplot(outlier.colour='red', outlier.shape=NA,outlier.size=0)+ theme(axis.text.x = element_text(angle = 90))+ylim(-0.5,0.5))+labs(title="Orléans"))
#print(boxplotDist)
ggsave(filename=paste("graphique/boxplotDistParAnnee","Orleans",".png",sep=""),device="png")
```
